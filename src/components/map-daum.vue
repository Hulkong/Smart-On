<template>
  <div id="ddaumAPI" class="mapArea">
    ...
    <div class="custom_zoomcontrol radius_border 4menu">
      ...
      <span @click="reload">
        <img
          src="/src/assets/images/map/map_util_refresh.png"
          class="zoom"
          alt="지도 초기화"
          width="15px"
          height="17px"
...
export default {
  watch: {
    zoomlevel: function(newLevel, oldLevel) {
      // zoomlevel 변경
      if (newLevel < 14 && newLevel > 1) this.map.setLevel(newLevel);
      else this.zoomlevel = oldLevel;
    },
    mapType: function(newType, oldType) {
      // base map type 변경
      this.map.setMapTypeId(kakao.maps.MapTypeId[newType]);
    }
  },
  mounted: function() {
    // 지도 생성
  ...
  methods: {
    createMap: function() {
      ...
      var mapOption = {
        center: new kakao.maps.LatLng(this.init.center[0], this.init.center[1]),
        disableDoubleClick: true,
        disableDoubleClickZoom: true,
        tileAnimation: true,
        level: this.zoomlevel // 지도의 확대 레벨
      };
    ...
    reload: function() {
      ...
      // 초기중심으로 이동
      this.map.setCenter(
        new kakao.maps.LatLng(this.init.center[0], this.init.center[1])
      );
      // 초기 Zoom level로 이동
      this.map.setLevel(this.init.level);
    }
  }
};
</script>
...
/* .custom_zoomcontrol.3menu {
  height: 120px;
}
.custom_zoomcontrol.4menu {
  height: 160px;
} */
...
.custom_zoomcontrol span img.zoom {
  width: 15px;
  height: 15px;
  padding: 12px 0;
  border: none;
...
.custom_zoomcontrol span:last-child img {
  border: none;
}
